...
class ChestXPert extends React.Component {
  state = {
    ...
    socket: null,
    classification: [
      { class_name: 'Cardiomegaly', probability: 30 },
      { class_name: 'Pneumonia', probability: 20 },
      { class_name: 'Pneumothorax', probability: 10 },
    ],
  };
  ...
  handleResponse(response) {
    ...
    let classification = JSON.parse(response['classification']);

    this.setState({
      disease: classification[0].class_name,
      accuracy: classification[0].probability.toFixed(2),
        ...
        >
          ...
          {this.state.loading && (
            <Box sx={{ width: '60%', margin: 'auto', marginTop: 5 }}>
              <div className="flex justify-center mb-10">
                <img style={{ width: '100%' }} src="/loading-video.gif" />
              </div>
          ...
          {this.state.hasResponse && (
            <>
              ...
              {/*  <Typography>More Predictions</Typography>*/}
              {/*</Box>*/}
              <Divider sx={{ margin: '30px auto', width: '80%' }} />
