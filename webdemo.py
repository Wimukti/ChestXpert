...
# React Component
_comp_func = components.declare_component(
    name="file_browser",
    # path="file-browser/dist",
    # url="http://localhost:3000/"
    url="https://cxr-report.web.app/"
)


def file_browser(upload_image, att_maps, resized_img):
    return _comp_func(upload_image=upload_image, att_maps=att_maps, resized_img=resized_img)
...
if 'att_maps' not in st.session_state:
    st.session_state['att_maps'] = None
...
def main():
    ...
    if uploaded_file:
        ...
        img_array = skio.imread(uploaded_file, as_gray=True)[None, ..., None]
        ...
        session.att_maps = attention_images

        file_browser(None, session.att_maps, session.upload_image)
