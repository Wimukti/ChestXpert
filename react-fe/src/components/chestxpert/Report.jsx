...
export default function Report({ report, setMainRadiologyOpinion }) {
  ...
  const validate = () => {
    setLoading(true);
    axios
      .post(endpoint, { field1: report, field2: radiologyOpinion })
      .then((res) => {
  ...
  return (
    <div style={{ marginBottom: 15, paddingLeft: '10%', paddingRight: '10%' }}>
      ...
      <div style={{ padding: '0px 40px', marginTop: 10, fontSize: 20, fontWeight: 'bold' }}>{report}</div>
      <div style={{ fontWeight: 'bold', marginTop: 20, padding: '0px 20px' }}>
        ...
        {addOpinion && (
          <div style={{ padding: '0px 20px', marginTop: 10 }}>
            <Button
              disabled={report === radiologyOpinion}
              onClick={() => {
            ...
            <textarea
              value={radiologyOpinion}
              onChange={(e) => handleChange(e)}
              style={{ width: '100%', padding: 10 }}
              rows={8}
              placeholder="Radiologist Opinion"
            ></textarea>
            <LoadingButton
              loading={loading}
              disabled={radiologyOpinion == ''}
              onClick={validate}
              size="small"
              sx={{
                marginBottom: '10px',
